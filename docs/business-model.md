# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Japan

## Classification

- Repository: `cloud-itonami-iso3166-jpn`
- ISO 3166: `JPN` (Japan)
...
- an already-incorporated `cloud-itonami-cofog-{code}` /
  ...
  `cloud-itonami-{ISIC}` operator wanting to bid on a Japanese
  public contract
...
- registration walkthrough for the Government Electronic Procurement System (GEPS) and the unified central-government tender qualification (全省庁統一資格)
- business/tax registration checklist: a Corporate Number (法人番号, National Tax Agency) plus any required municipal business registration; Qualified Invoice Issuer registration under Japan's consumption-tax invoice system (インボイス制度)
- local-content / preferential-procurement navigation: no blanket local-content quota, but most tenders require a Japan-resident authorized representative and Japanese-language documentation
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Japan).
